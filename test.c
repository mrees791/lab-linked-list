...
int main() {
	...
	verify(list_remove(list, 0) != true,
			"list_remove did not return true on valid index");

	verify(list_size(list) != 0,
			"list_size did not return 0 after list_remove");
    verify (list_find (list, 10) != -1, "list_find found 10 in list after list_remove");
	verify(list_get(list, 0) != -1,
			"list_get did not return error on invalid index");
	...
	verify(list_get(list, 4) != 35,
			"list_get did not return 35 as fifth element");
	...
	verify(list_get(list, 8) != 60,
			"list_get did not return 60 as ninth element");
	...
	verify(list_remove(list, 0) != true,
			"list_remove returned false when removing first element");
	...
	verify(list_get(list, 3) != 40,
			"list_get did not return 40 after list_remove");
